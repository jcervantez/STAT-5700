...
pacman::p_load(randomForest, tree, ISLR, rpart, rattle, pROC, partykit, ggplot2, glmnet, leaps, dplyr, keras, neuralnet, imager, ranger, xgboost, caret, data.table, tm, pander, RColorBrewer, wordcloud)
```
...
fit1 = lm(y ~ PC1_asvab + PC2_asvab + Gender, data=data)
summary(fit1)
```
...
\textbf{Part d:} We predict Michelle's log income to be 9.959322. This translates to $21,148.45.
...
```{r}
fit2 = rpart(y.train ~., data.train, minsplit=20, cp=.009)
plot(as.party(fit2), main="Final Tree with Rpart") 
```
    
\textbf{Part b:} This tree has 11 terminal nodes. Gender is the most important variable, while FatherEd is the least important variable. In this tree, we first split on Gender. For females, we split by Education followed by AFQT, while for males, we then split by Arith followed by years of Education.
...
test.error.fit1 <- mean((predict(fit1, data.test) - y.test)^2)
test.error.fit1

print("Fit 2 test error: ")
test.error.fit2 <- mean((predict(fit2, data.test) - y.test)^2)
test.error.fit2

print("Fit 1 train error: ")
train.error.fit1 <- mean((predict(fit1, data.train) - y.train)^2)
train.error.fit1

print("Fit 2 train error: ")
train.error.fit2 <- mean((predict(fit2, data.train) - y.train)^2)
train.error.fit2
```
\textbf{Part d:} By pruning fit2, we obtain the same testing MSE of 0.8211417.

```{r}
fit2.p = prune(fit2, cp=0.001)
summary(fit2.p)
plot(as.party(fit2.p), main="Pruned Tree with Rpart") 
print("Fit 2 pruned test error: ")
test.error.fit2p <- mean((predict(fit2.p, data.test) - y.test)^2)
...
y.boot.1 <- y.train[index1]
boot.1.tree <- rpart(y.boot.1 ~., boot.data1, minsplit=20, cp=.009)
...
y.boot.2 <- y.train[index2]
boot.2.tree <- rpart(y.boot.2 ~., boot.data2, minsplit=20, cp=.009)
...
oob_errors <- c()
for (m in mtry_vals) {
  fit <- randomForest(y.train ~ ., data=data.train, mtry=m, ntree=500)
  oob_errors <- c(oob_errors, fit$mse[500])
}
mtry_opt <- mtry_vals[which.min(oob_errors)]

fit4 <- randomForest(y.train ~ ., data=data.train, mtry=mtry_opt, ntree=500, importance=TRUE)
oob_error <- sqrt(fit4$mse[500])
test_error <- sqrt(mean((predict(fit4, data.test) - data.test$y.train)^2))
...
fit5_test_error <- sqrt(mean((bagged_preds - y.train)^2))
print(paste("Bagged Model Test Error:", round(fit5_test_error,3)))

# Calculate test errors for fit1, fit2, fit3, and fit4
fit1_test_error <- sqrt(mean((predict(fit1, data.test) - y.train)^2))
fit2_test_error <- sqrt(mean((predict(fit2, data.test) - y.train)^2))
fit3_test_error <- sqrt(mean((predict(fit3, data.test) - y.train)^2))
fit4_test_error <- sqrt(mean((predict(fit4, data.test) - y.train)^2))
...
dtrain <- xgb.DMatrix(train_data, label=y.train)
dtest <- xgb.DMatrix(test_data, label=y.test)
...
fit6_test_error <- sqrt(mean((predict(fit6, dtest) - y.test)^2))
...
# Convert data.valid to matrix
valid_data <- as.matrix(sapply(data.valid[,-ncol(data.valid)], as.numeric))
dvalid <- xgb.DMatrix(valid_data, label=y.valid)

# Evaluation on validation set
valid_error <- sqrt(mean((predict(fit6, dvalid) - y.valid)^2))
print(paste("Final Model (XGBoost) Validation Error:", round(valid_error,3)))


```

\begin{itemize}
  \item The best possible final model to predict income is the XGBoost model fit6. This model has the lowest test error among all the models built. 
\end{itemize}
...
```
\begin{itemize}
  \item The sparsity of the dtm is 0.973, which means that 97.3\% of the cells in the matrix are zero. This indicates that most documents contain only a small subset of the total terms.
\end{itemize}
...
data2 <- cbind(rating = yelp_data$rating, as.data.frame(as.matrix(dtm)))
```
...
# Divide the dataset into training/testing data (18,000 reviews) and validation data (2,000 reviews)
set.seed(99)
data_train <- data2[sample(nrow(data2), 13000, replace = FALSE), ]
data_not_train <- anti_join(data2, data_train)

set.seed(999)
data_test <- data_not_train[sample(nrow(data_not_train), 5000, replace = FALSE), ]
data_validate <- anti_join(data_not_train, data_test)

# Prepare Y and X matrix
Y <- as.matrix(as.numeric(data_train[,1])) # Extract Y
X <- as.matrix(data_train[,-1]) # Create the design matrix

# Set the seed
set.seed(100)

# Fit the Lasso model
fit_lasso <- cv.glmnet(X, Y, alpha = 1, nfolds = 15, intercept = T)
...
data_subset <- data2[,c("rating", var_1se)] 

# Run a logistic regression
fit_logit <- glm(rating ~ ., data = data_subset, family = binomial)
```
...
# Extract positive coefficients
fit_logit.coef <- coef(fit_logit)
fit_logit.coef <- fit_logit.coef[-1] # Remove the intercept
pos_coef <- fit_logit.coef[fit_logit.coef > 0]

# Rank the coefficients in decreasing order
pos_coef <- pos_coef[order(pos_coef, decreasing = T)]

# Get the leading 2 words and coefficients
top_2_pos_coef <- pos_coef[1:2]

# Print the leading 2 words and coefficients
pander(top_2_pos_coef, caption = "Leading 2 words with positive coefficients from logistic regression")
...
```{r}
# Extract the top 100 positive words
pos_words <- names(pos_coef)

# Create a word cloud
color_scheme <- brewer.pal(8,"Dark2")  # set up a pretty color scheme
wordcloud(pos_words[1:100], pos_coef[1:100], colors = color_scheme, ordered.colors = F)
```

From the word cloud and the leading 2 words with positive coefficients, we can see that words tend to fall into two categories. The first are adjectives which describe how incredible the experience is ("phenomenal", "amazing", "awesome"). The second are words that describe the quality of the service ("professional", "thorough"). This suggests that both the quality of the service and the experience are important factors in determining a positive review.
...
neg_coef <- fit_logit.coef[fit_logit.coef < 0]

# Rank the coefficients in increasing order
neg_coef <- neg_coef[order(neg_coef, decreasing = F)]

# Get the leading 2 words and coefficients
top_2_neg_coef <- neg_coef[1:2]

# Print the leading 2 words and coefficients
pander(top_2_neg_coef, caption = "Leading 2 words with positive coefficients from logistic regression")

# Extract the top 100 positive words
neg_words <- names(neg_coef)

# Invert coefficients for weights
neg_coef_invert <- round(neg_coef * -1,4)

# Create a word cloud
color_scheme <- brewer.pal(6,"Dark2")  # set up a pretty color scheme
wordcloud(neg_words[2:100], neg_coef_invert[2:100], colors = color_scheme, ordered.colors = F)

```

From the word cloud and the leading words with negative coefficients, we can see that negative words **also** tend to fall into two categories. The first are adjectives which describe how terrible the experience is ("worst", "horrible", "rude"). The second are words that describe the quality of the service ("ignore", "unprofessional"). This suggests that both the quality of the service and the experience are important factors in determining a negative review.

d) Summarize the findings. 

Both positive and negative word clouds show that the **quality of the service** and the **overall experience** are important factors in determining a review. Positive reviews tend to use words that describe how incredible the overall experience is and how professional the service is. Negative reviews tend to use words that describe how terrible the overall experience is and how unprofessional the service is. The data supports the hypothesis that each of these two factors are important. 
...
data_test_x <- as.matrix(data_test[,-1])
data_test_y <- as.matrix(data_test[,1])

# Predict the probabilities for the test data
prob_lasso <- predict(fit_lasso, data_test_x, s = "lambda.1se", type = "response")
prob_logit <- predict(fit_logit, data_test[,-1], type = "response")

# Convert probabilities to predictions based on majority vote
pred_lasso <- ifelse(prob_lasso > 0.5, 1, 0)
pred_logit <- ifelse(prob_logit > 0.5, 1, 0)

# Calculate the testing errors
test_error_lasso <- mean(pred_lasso != data_test_y)
test_error_logit <- mean(pred_logit != data_test_y)

# Create table for testing errors
testing_errors <- data.frame(Model = c("Lasso", "Logistic Regression"), Testing_Error = c(test_error_lasso, test_error_logit))

pander(testing_errors, caption = "Testing errors for Lasso and Logistic Regression models")
```

See the table above for the testing error for lasso and regression. The testing error for the logistic regression (using a Relaxed Lasso) is smaller than the testing error for the Lasso.

## 3. Random Forest  

i. Briefly summarize the method of Random Forest

Random Forest is an ensemble learning method that combines multiple decision trees to make more accurate classifications. Each decision tree is trained on a random subset of data and features. Each tree makes its own prediction, and the final prediction is made by taking a majority vote of all the trees. Random Forest typically outperforms single decision trees by reducing overfitting (by being more robust to noise) and increasing accuracy. However, it becomes hard to interpret the model.
...
data_train_rf <- read.csv("data_train.csv")
data_test_rf <- read.csv("data_test.csv")

# Set seed
set.seed(80)

# Fit random forest model
fit_rf <- randomForest(rating~., data_train_rf, mtry=3, ntree = 500)

# Plot the error rate
plot(fit_rf)

# Predict the probabilities for the test data
prob_rf <- predict(fit_rf, data_test_rf, type="response") # majority vote
pred_rf <- ifelse(prob_rf > 0.5, 1, 0)

test_error_rf <- mean(data_test_rf[,1] != pred_rf) 

# Add the training error to the table
testing_errors <- rbind(testing_errors, c("Random Forest", test_error_rf))

# Print table with Random Forest testing error added
pander(testing_errors, caption = "Testing errors for Lasso, Logistic Regression, and Random Forest models")

```

The testing error of the random forest is in the table above. My computer did not have enough computational power to calculate a random forest with a large number of trees. 

If I had access to more computational power, I would have used cross-validation to tune the `mtry` and `ntree` parameters, using the Out Of Bag errors to estimate the testing error of the random forest parameters.
...
dtrain = xgb.DMatrix(data = select(data_train, -rating) %>% data.matrix,
                     label = pull(data_train, rating))

# create a DMatrix object for the test data
dtest = xgb.DMatrix(data = select(data_test, -rating) %>% data.matrix,
                    label = pull(data_test, rating))

# set the parameters for the XGBoost model

params_custom = list(booster = 'gbtree', # select gradient tree boosting
              eta = .1, # shrinkage tuning parameter
              max_depth = 3, # max depth of a tree
              subsample = 0.7, # subsample ratio for observations
              gamma = 1, # penalty on number of leaves in a tree
              lambda = 1, # penalty on score 2-norm. 
              colsample_bytree = sqrt(ncol(data_train)-1) / sqrt(ncol(data_train)-1), # subsample ratio for features. 
              min_child_weight = 1, # minimum sum of instance scores in a leaf. 
              objective = 'reg:squarederror')

# make function print model's evaluation
xgb.fit = xgboost(data=dtrain, params = params_custom, 
                  nrounds = 200,    # max number of iterations, i.e. trees grown
...
prob_xboost <- plogis(pred)

# Convert probabilities to predictions based on majority vote
pred_xboost <- ifelse(prob_xboost > 0.5, 1, 0)

# Calculate the testing error
test_error_xboost <- mean(pred_xboost != data_test_y)

# Combine test errors with previous table and print
testing_errors <- rbind(testing_errors, data.frame(Model = "XGBoost", Testing_Error = test_error_xboost))

pander(testing_errors, caption = "Testing errors for Lasso, Logistic Regression, Random Forest, and XGBoost models")
```
...
# Combine the predictions of the models Lasso, Logit, and XGBoost
prob_lasso_logit_xboost <- (prob_lasso + prob_logit + prob_xboost) / 3
pred_lasso_logit_xboost <- ifelse(prob_lasso_logit_xboost > 0.5, 1, 0)

# Calculate the testing error
test_error_lasso_logit_xboost <- mean(pred_lasso_logit_xboost != data_test_y)
pander(test_error_lasso_logit_xboost)

# Combine the predictions of the models Lasso and Logit
prob_lasso_logit <- (prob_lasso + prob_logit) / 2
pred_lasso_logit <- ifelse(prob_lasso_logit > 0.5, 1, 0)

# Calculate the testing error
test_error_lasso_logit <- mean(pred_lasso_logit != data_test_y)
pander(test_error_lasso_logit)

# Combine the predictions of the models Lasso and XGBoost
prob_lasso_xboost <- (prob_lasso + prob_xboost) / 2
pred_lasso_xboost <- ifelse(prob_lasso_xboost > 0.5, 1, 0)

# Calculate the testing error
test_error_lasso_xboost <- mean(pred_lasso_xboost != data_test_y)
pander(test_error_lasso_xboost)

# Combine the predictions of the models Lasso and Random Forest and Logit
prob_lasso_rf_logit <- (prob_lasso + prob_rf + prob_logit) / 3
pred_lasso_rf_logit <- ifelse(prob_lasso_rf_logit > 0.5, 1, 0)

# Calculate the testing error
test_error_lasso_rf_logit <- mean(pred_lasso_rf_logit != data_test_y)
pander(test_error_lasso_rf_logit)

# Combine the predicions of all models
prob_all <- (prob_lasso + prob_logit + prob_rf + prob_xboost) / 4
pred_all <- ifelse(prob_all > 0.5, 1, 0)

# Calculate the testing error
test_error_all <- mean(pred_all != data_test_y)
pander(test_error_all)
```
...
The classifier that seems to produce the least testing error is the relaxed LASSO using a logistic regression. I am a bit surprised, because I thought the XGBoost and the Random Forest classifers would perform better. However, this might be because the parameters were not tuned sufficiently, as I did not have enough computational power to run the cross-validation with a large number of trees. 

```{r}

# Calculate the validation error for the relaxed LASSO using a logistic regression
prob_logit_validation <- predict(fit_logit, data_validate, type = "response")

# Convert probabilities to predictions based on majority vote
pred_logit_validation <- ifelse(prob_logit_validation > 0.5, 1, 0)

# Calculate the validation error
validation_error_logit <- mean(pred_logit_validation != data_validate[,1])

# Print the validation error
pander(validation_error_logit)
```

The final model is the relaxed LASSO using a logistic regression. The validation error is 0.1113, which is basically equivalent to the testing error of 0.1136. This is pretty remarkable! 
