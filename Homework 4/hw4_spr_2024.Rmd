...
Y <- as.matrix(data_train[,1]) # Extract Y
...
# Rename "..." as "ellipsis" to avoid errors
names(data_subset)[names(data_subset) == "..."] <- "ellipsis"
...
prob_lasso <- predict(fit_lasso, data_test_x, type = "response")
...
# Output training dataset for Random Forest
write.csv(data_train, "data/data_train.csv", row.names = F)
...
xgb.fit = xgboost(data=dtrain, params = params_custom, 
                  nrounds = 500,    # max number of iterations, i.e. trees grown
...
The classifier that seems to produce the least testing error is the relaxed LASSO using a logistic regression. 
