...
#### part a ####
################
# pca_scores <- unscaled_svd_out$u[, 1:4] * unscaled_svd_out$d[1:4]
pca_scores <- unscaled_svd_out$v
...
kmeans_result <- kmeans(pca_scores, centers = k_opt, nstart = 25)

colnames(pca_scores) <- c("PC1", "PC2", "PC3", "PC4")

# Now when you create pca_scores_df, the columns will have the correct names
pca_scores_df <- data.frame(pca_scores, Cluster = as.factor(kmeans_result$cluster), Subtype = brca_subtype)

# Now the ggplot code should work without the 'object not found' error
# Corrected ggplot code
ggplot(pca_scores_df, aes(x = PC1, y = PC2, color = Subtype, shape = Cluster)) +
  geom_point() +
...
sil_width_pca <- silhouette(kmeans_result$cluster, dist(pca_scores))
...
<<<<<<< HEAD
# Project the x patient to the space of PC1 and PC2
=======
proj = t(as.matrix(as.numeric(x_log1p))) %*% pca_scores[, c(1,2)]
proj
>>>>>>> 336e545184f0f7472fe36680ff29858cb09f8c60
