...
pacman::p_load(ISLR, tidyverse, data.table, Hmisc, corrplot, ggbiplot, factoextra, patchwork, cluster, gridExtra, skimr, GGally, broom, lmtest) # add the packages needed!!!
```
...
\textcolor{red}{\textbf{Answer:}} There are no missing values as there are no null values.

```{r}
data <- read.csv("data/NLSY79.csv", header = T, stringsAsFactors = F)
#str(data)
#summary(data)
#levels(as.factor(data$Job05))
#table(as.factor(data$Job05))
#sum(is.na(data))
```
...
\textcolor{red}{\textbf{Answer:}} For all of the Esteem variables, the minimum value is 1 and the maximum value is 4, which is as expected. There are no missing values. There is nothing to fix.

```{r}
esteem = select(data, contains("Esteem87"))
summary(esteem)
sum(is.na(esteem))
```
...
\textcolor{red}{\textbf{Answer:}} See .rmd document.
...
\textcolor{red}{\textbf{Answer:}} After correcting the responses such that higher scores correspond to higher self-esteem, we see that all of the questions have more weight towards the right, meaning that most participants gave responses that indicate higher self-esteem. However, questions 6 through 9 have a mass that is more centered around the middle, indicating that respondents have slightly lower self-esteem for these questions.
```{r}
summary(esteem)

esteem_long <- tidyr::gather(esteem, key = "Variable", value = "Value")

# Set up a layout
options(repr.plot.width = 12, repr.plot.height = 8)

# Generate histograms using ggplot
ggplot(esteem_long, aes(x = Value)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black") +
  facet_wrap(~Variable, scales = "free") +
  ggtitle("Histograms for Responses to all Questions") +
  xlab("Value") +
  ylab("Frequency")
...
\textcolor{red}{\textbf{Answer:}} All esteem scores are positively correlated; the minimum value in each row is positive. Questions that are adjacent to each other (e.g., questions 1 and 2 or questions 6 and 7) tend to have stronger correlations than questions that are further away from each other.

```{r}
corr_table = cor(esteem)

# see minimum correlation for each row
apply(corr_table, 1, FUN=min)

# summary statistics of correlations
summary(corr_table)

corrplot(corr_table, method="color")
```
...
4. PCA on 10 esteem measurements. (centered but no scaling)
    ...
    \textcolor{red}{\textbf{Answer:}} The PC1 loading is (-0.235, -0.244, 0.279, -0.261, 0.312, -0.313, -0.299, 0.393, 0.398, 0.376). The PC2 loading is (0.374, 0.367, -0.149, 0.321, -0.131, 0.209, 0.163, 0.332, 0.578, 0.260). As shown in the table below, PC1 and PC2 both are unit vectors and are orthogonal to each other.
    
```{r}
  pca <- prcomp(esteem, scale=FALSE, center=T)
  names(pca)
  
  # get loadings of PCA loadings
  pca_loadings = pca$rotation[, c(1,2)]
  knitr::kable(pca_loadings)
  
  # check the loadings are unit vectors and all orthogonal
  unit_orth = round(t(pca$rotation) %*% pca$rotation)[c(1,2), c(1,2)]
  knitr::kable(unit_orth)
  
  # plot bar plot of PCA
  plot(pca)
...
```
    ...
    b) Are there good interpretations for PC1 and PC2? (If loadings are all negative, take the positive loadings for the ease of interpretation)
     ...
     \textcolor{red}{\textbf{Answer:}} We can interpret PC1 as the difference between the total score for Questions 3, 5, 8, 9, 10 and the total score for Questions, 1, 2, 4, 6, 7. We do not see a good interpretation for PC2.
  
    c) How is the PC1 score obtained for each subject? Write down the formula.
    
     \textcolor{red}{\textbf{Answer:}} $$PC1 = -0.235 \times (Q1_score) + -0.244 \times (Q2_score) + 0.279 \times (Q3_score) + -0.261 \times (Q4_score) + 0.312 \times (Q5_score) + -0.313 \times (Q6_score) + -0.299 \times (Q7_score) + 0.393 \times (Q8_score) + 0.398 \times (Q9_score) + 0.376 \times (Q10_score$$.
    
    d) Are PC1 scores and PC2 scores in the data uncorrelated? 
    
    \textcolor{red}{\textbf{Answer:}} Yes, the PC1 and PC2 scores are uncorrelated. As shown in the table, the covariance of PC1 and PC2 is 0, implying that their correlation is 0.
    
    ```{r, results="markup"}
    # get correlation between PCA scores
    cov_matrix = round(cov(pca$x), 4)[c(1,2),c(1,2)]
    knitr::kable(cov_matrix)
    ```
    
    e) Plot PVE (Proportion of Variance Explained) and summarize the plot. 
    
    \textcolor{red}{\textbf{Answer:}} The plot shows that PC1 explains almost 50% of the variation and PC2 explains a little over 10% of the variance. We can also use this scree plot to determine the number of PCs that would be appropriate to use with our data. Using the elbow method, we see that using 2 PCs would be appropriate.
    ...
    \textcolor{red}{\textbf{Answer:}} Approximately 60% of variance in the data is explained by the first 2 PCs.
    ...
    \textcolor{red}{\textbf{Answer:}} The biplot shows that PC1 roughly corresponds to the difference between the total scores for Questions 3, 5, 8, 9, 10 and the total scores for Questions, 1, 2, 4, 6, 7; PC2 does not have a clear interpretation. We also see that the scores for Questions 1, 2, and 4 are highly correlated; the scores for Questions 3 and 5 are highly correlated; the scores for Questions 6 and 7 are highly correlated; and the scores for Questions 8 and 10 are also highly correlated. This supports are prior analysis from our above EDA, where we saw that questions that are closer to being "adjacent" with each other are more correlated with each other than are questions that are far away from each other. There still does not seem to be a clear interpretation of PC2, but PC2 seems to represent the rough difference between the total scores for questions 8, 9, 10 and questions 1-7.
    
    ```{r}
ggbiplot(pca, obs.scale = 2, var.scale = 1) + 
scale_color_discrete(name = '') +
labs(title = "Biplot of PCs") +
theme_bw()
...
5. Apply k-means to cluster subjects on the original esteem scores
    ...
    \textcolor{red}{\textbf{Answer:}} Using the elbow method, it appears that 2 is a reasonable number of clusters.
    
    ```{r}
    kmeans = fviz_nbclust(esteem, kmeans, method = "wss")
    kmeans
    ```
    
    b) Can you summarize common features within each cluster?
    
    \textcolor{red}{\textbf{Answer:}} Cluster 1 is characterized by higher scores for questions 3, 5, 8, 9, and 10. Cluster 2 is characterized by higher scores for questions 1, 2, 4, 6, and 7. This seems to roughly correspond to PC1, which can be interpreted as the approximate difference betwen the total score for questions 3, 5, 8, 9, and 10 and the total score for questions 1, 2, 4, 6, and 7.
    
    ```{r, results="markup"}
    esteem.kmeans = kmeans(esteem, centers = 2, nstart=10)
    
    esteem$cluster = esteem.kmeans$cluster

    mean_table = aggregate(esteem, by = list(esteem$cluster), FUN = mean)
    ...
    c) Can you visualize the clusters with somewhat clear boundaries? You may try different pairs of variables and different PC pairs of the esteem scores.
     ...
     \textcolor{red}{\textbf{Answer:}} When we cluster by PC1 and PC2, we see a very clear boundary given by PC1. Specifically, group 1 is almost entirely to the left of PC1 = 0, while group 2 is almost entirely to the right of PC2 = 0.
...
6. We now try to find out what factors are related to self-esteem? PC1 of all the Esteem scores is a good variable to summarize one's esteem scores. We take PC1 as our response variable. 
    ...
    a) Prepare possible factors/variables:
      ...
      - You may use PC1 of ASVAB as level of intelligence
       ...
       \textcolor{red}{\textbf{Answer:}} See .rmd file for code. I created a dataframe that contains all of the above listed variables. Income87 has some negative values, so I use 0 for any negative values. Since we cannot take the log of 0, I use log1p, which takes the log(1 + [value]). I also merge in columns for PC1 of the Esteem scores and PC1 of the ASVAB scores, for which I ran PCA for the ASVAB data. 
      
```{r}
  # PCA of ASVAB

  intel_cols = c("AFQT", "Coding", "Auto", "Mechanic", "Elec", "Science", "Math", "Arith", "Word", "Parag", "Number")
  asvab = data %>% select(all_of(intel_cols))
  pca_asvab <- prcomp(asvab, scale=FALSE, center=T)
  # get loadings of PCA loadings
  pc1_asvab = pca_asvab$rotation[, 1]

  esteem_cols = names(select(data, contains("Esteem87")))

  # Calculate log(Income87). There are some negative income values so we use 0 as the lowest possible income (so we can use log1p)
  data = data %>%
    mutate(LogIncome87 = log1p(pmax(0, Income87)))
  
  data$PC1_Esteem = as.vector(as.matrix(data[,esteem_cols]) %*% pca$rotation[,1])
  data$PC1_asvab = as.vector(as.matrix(data[,intel_cols]) %*% pc1_asvab)
  
  # Calculate BMI and convert weight to kg and height to m.
  data$bmi = (data$Weight05 * 0.453592) / (data$HeightFeet05 * 0.3048)^2
  
  # select relevant columns and convert Imagazine, Inewspaper, and Ilibrary to factors
  data87 = data %>%
    mutate_at(c("Gender", "Imagazine", "Inewspaper", "Ilibrary"), as.factor) %>%
    select(c("Gender", "Education05", "LogIncome87", "Imagazine", "Inewspaper", "Ilibrary", "MotherEd", "FatherEd", "FamilyIncome78", "PC1_Esteem", "PC1_asvab"))
...
```{r}
model1 = lm(PC1_Esteem ~ Gender + Education05 + LogIncome87 + Imagazine + Inewspaper + Ilibrary + MotherEd + FatherEd + FamilyIncome78 + PC1_asvab, data87)
summary(model1)

par(mfrow=c(1,2), mar=c(5,2,4,2), mgp=c(3,0.5,0)) # plot(fit3) produces several plots
plot(model1, 1, pch=16) # residual plot
abline(h=0, col="blue", lwd=2)
plot(model1, 2) # qqplot

model2 = lm(PC1_Esteem ~ Gender + Education05 + LogIncome87 + Inewspaper + PC1_asvab, data87)
summary(model2)

par(mfrow=c(1,2), mar=c(5,2,4,2), mgp=c(3,0.5,0)) # plot(fit3) produces several plots
plot(model2, 1, pch=16) # residual plot
abline(h=0, col="blue", lwd=2)
plot(model2, 2) # qqplot

model3 = lm(PC1_Esteem ~ Gender + Education05 + LogIncome87 + PC1_asvab, data87)
summary(model3)

par(mfrow=c(1,2), mar=c(5,2,4,2), mgp=c(3,0.5,0)) # plot(fit3) produces several plots
plot(model3, 1, pch=16) # residual plot
abline(h=0, col="blue", lwd=2)
plot(model3, 2) # qqplot

model4 = lm(PC1_Esteem ~ Gender + Education05 + LogIncome87 + FamilyIncome78 + PC1_asvab, data87)
summary(model4)
...
# Scree Plot of CPVE
plot(summary(brca_pca)$importance[3, 1:10], ylab="Cumulative PVE", xlab="Number of PCs", main="Scree plot of Cumulative PVE ")
...
```{r, result="asis"}
features <- brca_transformed %>% select(-BRCA_Subtype_PAM50)

brca_kmeans <- kmeans(features, centers = 4, nstart = 10)

saveRDS(brca_kmeans, "data/brca_kmeans.rds")

# Apply k-means with 4 centers
set.seed(124) 
kmeans_result <- readRDS("data/brca_kmeans.rds")

# Add the cluster labels back to the dataset to compare with actual subtypes
brca_with_clusters <- brca_transformed %>%
  mutate(Cluster = as.factor(kmeans_result$cluster))

# Create a discrepancy table
table(brca_subtype, brca_with_clusters$Cluster)
...
```{r}
svd_out <- irlba::irlba(as.matrix(brca_transformed[,-1]), nv = 10)

# Calculate the proportion of variance explained by each component
singular_values <- svd_out$d
...
plot(variance_explained, type = "b", xlab = "Principal Component", ylab = "Variance Explained", main = "Scree Plot")

# Plot the cumulative variance to decide how many components to retain
plot(cumulative_variance, type = "b", xlab = "Principal Component", ylab = "Cumulative Variance Explained", main = "Cumulative Variance Plot")
...
```{r}
# Perform PCA on centered and scaled data
scaled_svd_out <- irlba::irlba(scale(brca_transformed[,-1], center = TRUE, scale = TRUE), nv = 10)
scaled_pca_data <- data.frame(PC1 = scaled_svd_out$u[, 1] * scaled_svd_out$d[1],
                              PC2 = scaled_svd_out$u[, 2] * scaled_svd_out$d[2])

# Perform PCA on centered but unscaled data
unscaled_svd_out <- irlba::irlba(scale(brca_transformed[,-1], center = TRUE, scale = FALSE), nv = 10)
unscaled_svd_original <- irlba::irlba(scale(brca[,-1], center = T, scale = F), nv = 10)
unscaled_pca_data <- data.frame(PC1 = unscaled_svd_out$u[, 1] * unscaled_svd_out$d[1],
                                PC2 = unscaled_svd_out$u[, 2] * unscaled_svd_out$d[2])

# Combine with subtype information
brca_subtype <- brca_transformed$BRCA_Subtype_PAM50
scaled_pca_data <- cbind(scaled_pca_data, Subtype = brca_subtype)
unscaled_pca_data <- cbind(unscaled_pca_data, Subtype = brca_subtype)

# Create ggplot objects for each PCA result
p1 <- ggplot(scaled_pca_data, aes(x = PC1, y = PC2, color = Subtype)) +
      geom_point() + ggtitle("Scaled Data")
p2 <- ggplot(unscaled_pca_data, aes(x = PC1, y = PC2, color = Subtype)) +
      geom_point() + ggtitle("Unscaled Data")
...
#### part a ####
################
pca_loadings <- unscaled_svd_out$u[, 1:4] * unscaled_svd_out$d[1:4]
pca_scores <- as.matrix(unscaled_svd_original$v)
# Find the optimal number of clusters using within sum of squares (Elbow Method)
wss <- sapply(1:10, function(k) {
  kmeans(pca_loadings, centers = k, nstart = 25)$tot.withinss
})
...
kmeans_result <- kmeans(pca_loadings, centers = k_opt, nstart = 25)

colnames(pca_loadings) <- c("PC1", "PC2", "PC3", "PC4")

# Now when you create pca_scores_df, the columns will have the correct names
pca_loadings_df <- data.frame(pca_loadings, Cluster = as.factor(kmeans_result$cluster), Subtype = brca_subtype)

# GGplot2 code
ggplot(pca_loadings_df, aes(x = PC1, y = PC2, color = Subtype, shape = Cluster)) +
  geom_point() +
  geom_point(data = data.frame(x = kmeans_result$centers[,1], y = kmeans_result$centers[,2]), 
             aes(x = x, y = y), color = "black", size = 3, inherit.aes = FALSE) +
  ggtitle("PCA and K-means Clustering Results")
...
kmeans_original <- kmeans(brca_transformed[,-1], centers = 5, nstart = 10)

# Calculate silhouette widths for the original data
sil_width_original <- silhouette(kmeans_original$cluster, dist(brca_transformed[,-1]))
avg_sil_score_original <- mean(sil_width_original[, "sil_width"])

## PCA-transformed data

# Calculate silhouette widths for the PCA-transformed data
sil_width_pca <- silhouette(kmeans_result$cluster, dist(pca_loadings))
avg_sil_score_pca <- mean(sil_width_pca[, "sil_width"])

# Create silhouette plots for both
p1 <- fviz_silhouette(sil_width_original) + ggtitle("Silhouette Plot for Original Data")
p2 <- fviz_silhouette(sil_width_pca) + ggtitle("Silhouette Plot for PCA-reduced Data")

# Arrange plots side by side
grid.arrange(p1, p2, ncol = 2)

# Print the average silhouette scores
print(paste("Average silhouette score for original data:", avg_sil_score_original))
print(paste("Average silhouette score for PCA-reduced data:", avg_sil_score_pca))
...
proj = t(as.matrix(as.numeric(x_log1p))) %*% pca_scores[, c(1,2)]


p <- ggplot(pca_loadings_df, aes(x = PC1, y = PC2, color = Subtype, shape = Cluster)) +
  geom_point() +
  geom_point(data = data.frame(x = kmeans_result$centers[,1], y = kmeans_result$centers[,2]), 
             aes(x = x, y = y), color = "black", size = 3, inherit.aes = FALSE) +
  ggtitle("PCA and K-means Clustering Results")
# Add the patient's projection with a black dot
p + geom_point(aes(x = proj[1], y = proj[2]), color = "black", size = 5)


patient_proj <- c(25.2, -3.5)  # Patient's PC1 and PC2
# Calculating Euclidean distances to each cluster centroid
distances <- apply(kmeans_result$centers[, 1:2], 1, function(centroid) {
  sqrt(sum((centroid - patient_proj)^2))
})
...
print(paste("The patient is closest to cluster:", closest_cluster))
```
...
What determines how fuel efficient a car is? Are Japanese cars more fuel efficient?  To answer these questions we will build various linear models  using 
...
```{r case-3-1a, include=FALSE}
auto <- ISLR::Auto
?ISLR::Auto
hist(auto$acceleration) # to check if there are some outliers

# Examine data variables and generate summaries
str(auto)
summary(auto) 
skim(auto)

# Set each variable to appropriate class
auto <- auto |>
  mutate(
    cylinders = as.integer(cylinders), # set cylinder as integer
    origin = as.factor(origin) # set origin as factor
    )
```
...
```{r case-3-1b, include=FALSE}

# Check number of unique rows in dataset
nrow(unique(auto))

# Check number of unique car names in dataset
...
\textcolor{red}{\textbf{Answer:}} 
There are 392 cars in the dataset (with 392 unique rows). There are also 301 unique car names in the dataset (that span different model years). 

c) EDA, focus on pairwise plots and summary statistics. Briefly summarize your findings and any peculiarities in the data.

```{r case-3-1c, echo=FALSE}
...
```{r case-3-2a, echo=FALSE}

# Fit OLS model
ols_1 <- lm(mpg ~ year, data = auto)
summary(ols_1)
```

\textcolor{red}{\textbf{Answer:}}
Yes, year is a significant variable at the .05 level. The coefficient of year is 1.23, which means that for each additional year, the `mpg` of a car increases by 1.23 on average. Note that this does not imply a causal link between `year` and `mpg`, but an association between the two variables.
...
\textcolor{red}{\textbf{Answer:}}
The difference in the 95% CI's for the coefficient of year between the two models is due to the presence of `horsepower` in the second model. In the first model, there is less precision and more noise in the model since the effect of `horsepower` isn't taken into account. This results in a wider confidence interval for the coefficient of `year` in the first model. In the second model, including `horsepower` in the model makes the model more precise, and so the we can estimate a narrower confidence interval for the coefficient of `year` is narrower.
...
```{r case-3-2d, echo=FALSE}

# Penalize scientific notation
options(scipen = 999)

# Fit OLS model with interaction
ols_3 <- lm(mpg ~ year * horsepower, data = auto)
summary(ols_3)
...
```{r case-3-3a, echo=FALSE}

# Regress mpg on cylinders
ols_4 <- lm(mpg ~ cylinders, data = auto)
...
\textcolor{red}{\textbf{Answer:}}
Yes, `cylinders` is significant at the .01 level. The coefficient of `cylinders` is -3.56, which means that for each additional cylinder, the `mpg` of a car decreases by 3.56 on average.

However, the interpretation of the model is difficult, becuase the intercept represents zero cylinders, which is not a meaningful value. Also, cylinders only take on discrete values.
...
```{r case-3-3b, echo=FALSE}

# Change cylinders into a factor
auto <- mutate(auto, cylinders = as.factor(cylinders))
levels(auto$cylinders)

# Check the count of each level in the factor
table(auto$cylinders)

# Regress mpg on cylinders (as a factor)
ols_5 <- lm(mpg ~ cylinders, data = auto)
...
\textcolor{red}{\textbf{Answer:}}
If `cylinders` is treated as a categorical variable, the OLS regression will treat each factor as a separate indicator variable. Only `cylinders4` is significant at the .01 level. The coefficient of `cylinders4` is 8.73, which means that the `mpg` of a car with 4 cylinders is 8.73 higher on average than a car with 3 cylinders. 

It makes sense that only `cyliners4` is significant, since the other levels of `cylinders` have very few observations.
...
\textcolor{red}{\textbf{Answer:}}
The fundamental difference between treating `cylinders` as a continuous and categorical variable is that the continuous model assumes a linear relationship between `cylinders` and `mpg`, while the categorical model assumes that each level of `cylinders` has a different effect on `mpg`. 
...
```{r case-3-4a, echo=FALSE}

# Fit an exploratory model with all terms to see which ones explain variation
ols_6 <- lm(mpg ~ cylinders + displacement + horsepower + weight + acceleration + year + origin + log(displacement) + log(horsepower) + log(weight) + log(acceleration) + log(year) + displacement^2 + horsepower^2 + weight^2 + acceleration^2 + year^2 + cylinders * displacement + cylinders * horsepower + cylinders * weight + cylinders * acceleration + cylinders * year + cylinders * origin + displacement * horsepower + displacement * weight + displacement * acceleration + displacement * year + displacement * origin + horsepower * weight + horsepower * acceleration + horsepower * year + horsepower * origin + weight * acceleration + weight * year + weight * origin + acceleration * year + acceleration * origin + year * origin, data = auto)

summary(ols_6)

# Re-center year so that terms are more interpretable
auto <- auto |>
  mutate(years_after_1970 = year - 70)


# Simplify the model to exclude variables that are not close to significant
# And exclude point estimates that are not large effects on mpg
ols_7 <- lm(mpg ~ log(weight) + log(horsepower) + years_after_1970 * log(horsepower), data = auto)
...
# Test if top part of QQ plot suggests that residuals are significantly right skewed
shapiro.test(residuals(ols_7))

# Residual plot
plot(ols_7, which = 1)

# Test for homoescedasticity, given residuals do not appear to be heteroscedastic
bptest(ols_7)
```
...
* `log(horsepower)` has a positive effect on `mpg`, with a coefficient of 80, which means that for every 1% increase in horsepower, `mpg` increase by 80. However, one should not interpret this isolation, because there is an interaction term with `year` that changes the effect of `log(horsepower)` on `mpg` depending on the year of the car.
...
* The interaction term between `years_after_1970` and `log(horsepower)` has a negative effect on `mpg`, with a coefficient of -1.1, which means that for every 1% increase in horsepower `mpg` decreases by 1.1 AND this effect manipulatively increases for each year after 1970. In essence, the effect of `log(horsepower)` on `mpg` is less negative for older cars and more negative for newer cars in the dataset.
...
```{r case-3-4c, echo=FALSE}

# Create a data frame with the car's characteristics
new_car <- data.frame(cylinders = 8, displacement = 350, horsepower = 260, weight = 4000, years_after_1970 = 13, origin = '1')

# Predict the mpg of the new car
predict(ols_7, new_car, interval = "confidence")

```

\textcolor{red}{\textbf{Answer:}}
The predicted `mpg` of the new car is 9.65 with a 95% confidence interval of (6.95, 12.3). 
