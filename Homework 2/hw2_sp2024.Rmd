...
# original data to get optimal number of clusters for kmeans

# wss <- sapply(1:10, function(k) {
#   kmeans(brca_transformed[,-1], centers = k, nstart = 10)$tot.withinss
# })
# 
# # Plot the elbow plot
# elbow_plot <- data.frame(NumClusters = 1:10, WSS = wss)
# ggplot(elbow_plot, aes(x = NumClusters, y = WSS)) +
#   geom_point() +
#   geom_line() +
#   theme_minimal() +
...
# # Calculate the distance matrix once, as it's computationally intensive
# dis_original <- dist(brca_transformed[,-1])^2
# dis_pca <- dist(pca_scores)^2
# 
# # Set up a plotting area
# par(mfrow=c(2,2))
# 
# # Loop through a range of k values for the original data
# for(k in 2:5) {
#   set.seed(124)  # For reproducibility
#   kmeans_original <- kmeans(brca[,-1], centers = 5, nstart = 10)
#   sil_width_original <- silhouette(kmeans_original$cluster, dis_original)
#   plot(sil_width_original, col = scales::hue_pal()(k), border = NA)
# }
# 
# # Loop through a range of k values for the PCA-reduced data
# for(k in 2:5) {
#   set.seed(124)  # For reproducibility
#   kmeans_pca <- kmeans(pca_scores, centers = k, nstart = 10)
#   sil_width_pca <- silhouette(kmeans_pca$cluster, dis_pca)
#   plot(sil_width_pca, col = scales::hue_pal()(k), border = NA)
# }
