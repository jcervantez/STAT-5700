...
pacman::p_load(ISLR, readxl, tidyverse, magrittr, dplyr, ggplot2, pander, broom, skimr)
...
\textcolor{red}{\textbf{Proposal:}}

To estimate the audience size of Wharton Business Radio Show (as of today). We will launch a study on [Prolific Academic](https://www.prolific.com/). According to the website, it costs approximately \$8/hr for each participant. Our survey should take 3-minutes, which means that it will cost \$0.40 per participant. We will recruit approximately 2,500 participants, for a total cost of \$1,000. We will use the following questions to estimate the Wharton Radio Show's audience size:

\begin{enumerate}
  \item Do you listen to podcasts? (Yes/No)
  \item What types of podcasts do you listen to?
  \begin{itemize}
    \item Business
    \item News
    \item Entertainment
    ...
    \item Other
  \end{itemize}
  \item Please select the podcasts that you actively listen to:
  \begin{itemize}
    \item Wharton Business Radio
    \item The Joe Rogan Show
    \item The Daily
    \item Arm Chair Expert
    \item Huberman Lab
    \item Lex Fridman Podcast
    \item Other [text entry]
  \end{itemize}
\end{enumerate}

After we ask these questions, we will calculating the proportion of Wharton listeners to total podcast listeners within the Prolific sample and applying this proportion to the known estimate of total 100 million podcast listeners\footnote{https://explodingtopics.com/blog/podcast-listeners} to estimate the Wharton show's audience size. We use the total podcast estimate because the Wharton audience is influenced by various sources (e.g., Apple podcasts, Spotify, etc.), and not just a single source (e.g., SiriusXM.)
...
\textcolor{red}{\textbf{Answer:}} I inspected the variable names by looking at head(wsci). Only "Field and sex" and "Degrees Awarded" are not the correct names, so I rename those.
...
\textcolor{red}{\textbf{Answer:}} I inspected the variable natures using str(wsci). Field, Degree, and Sex are strings, which is appropriate for their data. Year and Number are numeric, which are also appropriate.
```{r}
...
\textcolor{red}{\textbf{Answer:}}I checked for missing values by inspecting the summary of wsci, the table of values for each variable, and calculating the sum of all NA values. This reveals that there are no missing values. All columns have the same length, and there are no null values in any of the columns.
```{r}
...
# Use skim function to double-check NA values
skim(wsci)
```
...
\textcolor{red}{\textbf{Answer:}} I found the number of fields by looking at the length of the list of unique values in Field. There are 10 unique fields.
```{r}
...
\textcolor{red}{\textbf{Answer:}} I got the degree types by looking at the unique values in Degree. They are BS, MS, PhD.
```{r}
...
\textcolor{red}{\textbf{Answer:}} I found this by looking at the length of the list of unique values in Year. There are 11 years of statistics in this dataset.
```{r}
...
\textcolor{red}{\textbf{Answer:}} To examine this, I filtered the data to include just data on BS degrees from 2015. Then, I looked at summary statistics of the number of degrees earned by men and women and plotted the number of S&E and non-S&E degrees earned by men and women.
...
\textcolor{red}{\textbf{Answer:}} In summary, our data analysis shows that in general, more males pursue science-related fields. That is, across time and all three degree types, more men than women pursue science-related degrees. This is especially true for the fields related to engineering, data science, and "harder" sciences. However, when we consider "softer" sciences, such as psychology and social science, mroe women than men pursue these degrees. One exception to this rule is biological sciences, in which more women than men pursue these degrees.
...
```{r question_4.1b, warning=FALSE, message=FALSE}
# Read in .csv data using better tidyverse function read_csv :)
baseball <- read_csv("./data/baseball.csv", show_col_types = FALSE)

# Examine the data structure:
# str(baseball)
# head(baseball)

# Create a new table with the log difference
baseball_log_diff <- baseball |>
  group_by(team) |> # group by team in ascending order
  mutate(log_pay = log(payroll)) |> # create new variable log_pay
  mutate(diff_log = log_pay - lag(log_pay)) |> # create new variable diff_log
  ungroup() # ungroup the data

# Check table is correct:
# str(baseball_log_diff)
head(baseball_log_diff, 20)
```

\textcolor{red}{\textbf{Answer:}} 
Look to .rmd document for the code.
...
```{r question_4.1c}
# Create a new long data table
baseball_long <- baseball_log_diff |>
  select(team, year, diff_log, win_pct) |> # select the relevant columns
  filter(!is.na(diff_log)) # remove NA values for diff_log

# Check table is correct:
# str(baseball_long)
head(baseball_long, 20)

filter(baseball, team == "Pittsburgh Pirates", year >= 2010)
```
...
a). Which five teams had the highest increase in their payroll between years 2010 and 2014, inclusive?

```{r question_4.2a, results = 'markup', warning=FALSE, message=FALSE}
# Print the top 5 teams with the highest increase in win percentage
baseball_pay_summary <- baseball_long |>
  filter(year > 2010 & year <= 2014) |> # filter for years 2011 to 2014
  group_by(team) |> # group by team
  summarize(
    diff_log_2010_2014 = sum(diff_log), # sum diff_long to find total increase
    percent_change_2010_2014 = (exp(diff_log_2010_2014) - 1) * 100 # convert to percentage
  ) |>
  arrange(desc(diff_log_2010_2014)) # arrange in descending order

# Print the top 5 teams with the highest increase in payroll
baseball_pay_summary |>
  rename(
    `Delta Log(Payroll)` = diff_log_2010_2014, 
    `Percent Change in Payroll` = percent_change_2010_2014 # rename columns)
  ) |>
  head(5) |> # select the top 5 teams
  pander(caption = "Top 5 Teams with Highest Increase in Payroll from 2010-14") # print table in pretty format
```

\textcolor{red}{\textbf{Answer:}} 
The top five teams are the LA Dogdgers, the Texas Rangers, the San Diego Padres, the Pittsburg Pirates, and the Washington Nationals. (see table above)

\hrulefill

b). Between 2010 and 2014, inclusive, which team(s) "improved" the most? That is, had the biggest percentage gain in wins?

```{r question_4.2b, results='markup', warning=FALSE, message=FALSE}
# Create variable of the log difference in win percentage from the past year
baseball_long <- baseball_long |>
  group_by(team) |> # group by team in ascending order
  mutate(log_win_pct = log(win_pct)) |> # create new variable log_pay
  mutate(diff_log_win_pct = log_win_pct - lag(log_win_pct)) |> # create variable
  ungroup() # ungroup the data

# Create dataframe of changes in win percentage for teams between 2010 and 2014
baseball_win_summary <- baseball_long |>
  filter(year > 2010 & year <= 2014) |> # filter for years 2011 to 2014
  group_by(team) |> # group by team
  summarize(
    diff_log_win_pct_2010_2014 = sum(diff_log_win_pct), # sum diff_long to find increase
    percent_change_win_pct_2010_2014 = (exp(diff_log_win_pct_2010_2014) - 1) * 100 # convert to percentage
  ) |>
  arrange(desc(diff_log_win_pct_2010_2014)) # arrange in descending order

# Print the top 5 teams with the highest increase in win percentage
baseball_win_summary |> 
  rename(
    `Delta Log(Win %)` = diff_log_win_pct_2010_2014, 
    `Percent Change in Win %` = percent_change_win_pct_2010_2014) |> # rename columns)
  head(5) |> # select the top 5 teams
  pander(caption = "Top 5 Teams with Highest Increase in Win Percentage from 2010-14") # print table in pretty format
```

\textcolor{red}{\textbf{Answer:}} 
The Pittsburgh Pirates improved the most (see table above).
...
```{r question_4.3, results='markup', warning=FALSE, message=FALSE}
# Combine payroll and win percentage summaries
baseball_total_summary <- inner_join(
  baseball_pay_summary,
  baseball_win_summary,
  by = "team"
)

# Run regression of win percentage on yearly increase in payroll
ols_1 <- lm(diff_log_win_pct_2010_2014 ~ diff_log_2010_2014, data = baseball_total_summary)

# Print the regression summaries
pander(tidy(ols_1), caption = "Regression of difference in ln(win percentage) from 2010-2014 on difference in ln(payroll) from 2010-2014")

ggplot(baseball_total_summary, aes(x = diff_log_2010_2014, y = diff_log_win_pct_2010_2014)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "Win Percentage Increases vs. Payroll Increases (2010-2014)",
    x = "2010-2014 Difference in ln(payroll)",
    y = "2010-2014 Difference in ln(win percentage)"
  )
```

\textcolor{red}{\textbf{Answer:}} 
The table of statistics above and the corresponding graph provides some evidence that higher % changes in payroll on the log scale correlate with higher % changes in win percentage. However, it's important to note that even though higher % changes in payroll **predict** higher % changes in win percentage, it doesn't necessarily mean that higher % changes in payroll **cause** higher % changes in win percentage. This is because there may be ommited variables that are correlated with both payroll and win percentage – so we cannot conclusively say that one "leads" to the other. 

If we regress % change in win percentage on % change in payroll, we find that although the results directionally affirm our hypothesis, they are not statistically significant. In fact, the r-square for this model is extremely low, which makes it plausible that there there is omitted variables which would bias this estimate. This implies the results are only suggestive, and not conclusive.
...
\textcolor{red}{\textbf{Answer:}} 
This question isn't very clear as its not clear how "performance" is being defined (e.g., win percentage, percentage point improvement in win percentage, percentage increase in win percentage, etc.). 

**If we assume that "performance" is being defined as win percentage**, then we can compare the two models to see which variable better explains the variance in win percentage using the the R-Squared metric. Below, you can find two regressions: one with win percentage regressed on payroll, and another with win percentage regressed on the log difference in payroll. The R-Squared for the first model is 0.12, while the R-Squared for the second model is 0.03. This suggests that the yearly payroll is significantly better at explaining the variance in win percentage than the yearly increase in payroll.

```{r question_4.4a, results='markup'}
# Run regression of win percentage on yearly increase in payroll
lm_1 <- lm(win_pct ~ diff_log, data = baseball_log_diff)

# Run regression of win percentage on yearly payroll
lm_2 <- lm(win_pct ~ payroll, data = baseball_log_diff)

# Set options for scientific notation
options(scipen = 0)

# Print the regression summaries
pander(tidy(lm_1), caption = "Regression of win percentage on yearly increase in payroll")
pander(tidy(lm_2), caption = "Regression of win percentage on yearly payroll")
```

We can also look at another criterion to compare models against one another: the Bayesian Information Criterion (BIC). The BIC is a criterion for model selection among a finite set of models, and can provide a rough approximation to the logarithm of the Baye's factor (. The difference in BIC between the two models is 115, which is an incredibly large Bayes factor (approximately equal to $e^{(115/2)}$). This shows convincingly that payroll is better at explaining win percentage than yearly increase in payroll.

```{r question_4.4b, results='markup'}
# Calculate Bayesian Information Criterion for both models using glm and the identity link
glm_1 <- glm(win_pct ~ diff_log, family = gaussian(link = "identity"), data = baseball_log_diff)
glm_2 <- glm(win_pct ~ payroll, family = gaussian(link = "identity"), data = baseball_log_diff)


# Extract BIC from the models
payroll_BIC <- BIC(glm_2)
payroll_increase_BIC <- BIC(glm_1)
BIC_delta <- payroll_increase_BIC - payroll_BIC

# Create a table displaying the BIC for both models and the difference
pander(cbind(payroll_increase_BIC, payroll_BIC, BIC_delta), caption = "BIC comparison for both models")
```
