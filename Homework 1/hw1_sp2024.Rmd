...
table(d0$age)
d0 <- d0 |> 
  filter(!grepl("^\\d+$", age) | as.numeric(age) < 18 | as.numeric(age) > 100) |> 
  mutate(age = case_when(
    age == "27`" ~ 27,                        # Change '27`' to '27'
    age == "Eighteen (18)" ~ 18,              # Change 'Eighteen (18)' to '18'
    TRUE ~ NA_integer_                        # Set all other values to NA
  ))
...
```{r, include=FALSE}
paydata = read.csv("./data/MLPayData_Total.csv")
head(paydata)
...
```{r}
diff_log = baseball %>% filter(year %in% c(2012, 2013)) %>% mutate(logpay = log(payroll))
lag(diff_log$logpay)
baseball %>% filter(year %in% c(2012, 2013)) %>% group_by(team) %>% mutate(logpay = log(payroll)) %>% mutate(diff_log = lag(logpay))
```
